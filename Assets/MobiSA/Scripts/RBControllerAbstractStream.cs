...
{
    public abstract class RBControllerAbstractStream : MonoBehaviour {
        public readonly string unique_source_id;
        public readonly string StreamName;
        public readonly SteamVR_Controller.Device device;

        public RBControllerAbstractStream(string unique_id, string streamname, SteamVR_Controller.Device device)
        {
            this.unique_source_id = unique_id;
        ...
        if (device != null)
        {
            //Debug.LogWarning("[RBEyetracking] Got a Vive Controller, can build stream");
       ...
       private void pushSample()
       {
           if (outlet == null)
            ...
            if (device != null)
            {
                ...
                currentSample[0] = device.transform.pos.x;
                currentSample[1] = device.transform.pos.y;
                currentSample[2] = device.transform.pos.z;
                currentSample[3] = device.transform.rot.x;
                currentSample[4] = device.transform.rot.y;
                currentSample[5] = device.transform.rot.z;
                currentSample[6] = device.transform.rot.w;
