...
namespace Assets.MobiSA.Scripts
{
    ...
    public class ExperimentEditorWindow : EditorWindow
    {
        public Setup setup;
       ...
       [MenuItem("MoBI SA / Editing Trials List ")]
        public static void Init()
        {
            var window = (ExperimentEditorWindow) EditorWindow.GetWindow(typeof(ExperimentEditorWindow));
            window.Show();
        ...
        {
            trialname = 1;
            dataDirectory = Application.dataPath + "/MobiSA/Config/";
            expectedTrialsConfig = dataDirectory + "trialslist.json";
            saveTrialsConfig = dataDirectory + "trialslist_generated.json";
            ...
            {
                // ReinitTrials();
                MobiSACore.generatedTrials = trialsConfig.GenerateTrialsList(trialsConfig.listOfTrials);
        ...
        private void ReinitTrials()
        {
            ...
            trialnames = new List<String>();
            foreach (Trial t in trialsConfig.listOfTrials) {
                trialnames.Add(t.trial);
                trialsList.Add(t);
      ...
      /*  private void SaveTrials()
        ...
        public void OnGUI() {
            ...
            tIndex = EditorGUILayout.Popup(tIndex, trialnames.ToArray());
  
            Trial t = trialsList[tIndex];
            /*if (GUILayout.Button("-"))
            {
                //hacky trick
            ...
            //try{ 
                t.trial = EditorGUILayout.TextField("Trial", t.trial);
                t.instances = EditorGUILayout.IntField("Instance", t.instances);
                t.color = EditorGUILayout.ColorField("Color", t.color);
                t.heigth = EditorGUILayout.FloatField("Heigth", t.heigth);
                EditorGUILayout.BeginHorizontal();
                t.scaleAvg= EditorGUILayout.FloatField("Scale (Avg)", t.scaleAvg);
                t.scaleVar= EditorGUILayout.FloatField("Scale (Var)", t.scaleVar);
                EditorGUILayout.EndHorizontal();

                EditorGUILayout.BeginHorizontal();
                t.velocityAvg = EditorGUILayout.FloatField("Velocity (Avg)", t.velocityAvg);
                t.velocityVar = EditorGUILayout.FloatField("Velocity (Var)", t.velocityVar);
                EditorGUILayout.EndHorizontal();

                EditorGUILayout.BeginHorizontal();
                t.distanceAvg = EditorGUILayout.FloatField("Distance (Avg)", t.distanceAvg);
                t.distanceVar = EditorGUILayout.FloatField("Distance (Var)", t.distanceVar);
                EditorGUILayout.EndHorizontal();
          ...
          /*  }
                ...
                {
                    if (MobiSACore.config != null)
                    {
                        MobiSACore.config = new Config();
                        MobiSACore.config.experiment.maximumAngle = angle;
                        MobiSACore.config.experiment.parallelSpawns = numberOfParallelSpawns;
                        MobiSACore.config.experiment.pausetime = pausetime;
                        MobiSACore.config.experiment.pausetimeTimingJitter = pausetimeTimingJitter;
